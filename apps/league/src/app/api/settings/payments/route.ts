...
// GET /api/settings/payments — get payment provider settings for this tenant
export async function GET(): Promise<NextResponse> {
  ...
  const payments = tenantDoc.settings?.payments || {
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  ...
  // Initialize payments settings if not present
  ...
  // Build the provider config — don't overwrite secrets if masked value sent back
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  ...
  if (payments?.providers) {
    ...
    // If we removed the default, reset to stripe
