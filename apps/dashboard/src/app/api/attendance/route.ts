...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (existing > 0) {
    return NextResponse.json({ error: "Attendance already initialized for this event" }, { status: 409 });
  }

  const roster = await Roster.find({ teamId: event.teamId, status: "active" }).lean();
  if (roster.length === 0) {
    return NextResponse.json({ error: "No active players on this team's roster" }, { status: 400 });
  }
